...
class SwiGLU(torch.nn.Module):
    def __init__(self, hidden_dim, factor=4, activate_lora=False):
        super().__init__()
        ...
        self.lora_lin1 = LoraLayer(hidden_dim, factor * hidden_dim, name='lora_lin1', active=activate_lora)   # LoRA to lin1
...
class FeedForward(torch.nn.Module):
    def __init__(self, d_model, dropout_rate=0.1, activate_lora=False):
        super().__init__()
        self.swiglu = SwiGLU(d_model, activate_lora=activate_lora)
...
class LoraLayer(torch.nn.Module):
    def __init__(self, input_dim, output_dim, rank=8, alpha=32, active=False, name='lora'):
        super().__init__()
        self.rank = rank
        self.alpha = alpha
    ...
    def forward(self, x):
        if not self.active:          # fast exit
            # shape: (..., out_features) so downstream ops can't hide bugs
            zero = torch.zeros(*x.shape[:-1], self.B.out_features,device=x.device, dtype=x.dtype)
            return zero
        return self.B(self.A(x)) * self._scale

    
class MultiHeadAttention(torch.nn.Module):
    def __init__(self, d_model, num_heads, dropout=0.1, activate_lora=False):
        super().__init__()
        ...
        self.lora_wq = LoraLayer(d_model, d_model, name='lora_wq', active=activate_lora)
        self.lora_wv = LoraLayer(d_model, d_model, name='lora_wv', active=activate_lora)
    ...
    def forward(self, q, k=None, v=None, padding_mask=None, use_causal_mask=False):
        ...
        q = self.wq(q) + self.lora_wq(q)    # (B, T_q, d_model)
        k = self.wk(k)                      # (B, T_k, d_model)
        v = self.wv(v) + self.lora_wv(v)    # (B, T_v, d_model)
        ...
        # 3. ROTARY
        max_len = max(Tq, Tk)
        sin, cos = make_sincos(max_len, self.depth, device=q.device.type)  # depth = d_model / num_heads
        ...
        if use_causal_mask:
            causal = torch.triu(torch.ones(Tq, Tk, device=q.device.type), diagonal=1).unsqueeze(0).unsqueeze(0)  # (1,1,T_q,T_k)
...
class CausalSelfAttention(torch.nn.Module):
    def __init__(self, d_model, num_heads, dropout=0.0, activate_lora=False):
        super().__init__()
        self.mha = MultiHeadAttention(d_model=d_model, num_heads=num_heads, dropout=dropout, activate_lora=activate_lora)
        self.rmsnorm = RMSNorm(d_model)
...
class DecoderLayer(torch.nn.Module):
    def __init__(self, d_model, num_heads, dropout_rate=0.1, activate_lora=False):
        super().__init__()
        self.causal_self_attention = CausalSelfAttention(num_heads=num_heads, d_model=d_model, dropout=dropout_rate, activate_lora=activate_lora)
        self.ffn = FeedForward(d_model, activate_lora=activate_lora)
...
class Decoder(torch.nn.Module):
    def __init__(self, num_layers, d_model, num_heads, input_vocab_size, dropout_rate=0.1, activate_lora=False):
        super().__init__()
        self.d_model = d_model
        ...
        self.dec_layers = torch.nn.ModuleList([
            DecoderLayer(d_model=d_model, num_heads=num_heads, dropout_rate=dropout_rate, activate_lora=activate_lora)
